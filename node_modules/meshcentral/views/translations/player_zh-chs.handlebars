<!DOCTYPE html><html lang="zh-chs" dir="ltr" xmlns="http://www.w3.org/1999/xhtml"><head>
    ...
    <script>
        ...
        var waitTickTimer = null;
        var waitTickStartWall = 0;
        var waitTickStartRec = 0;
        var waitTickTargetRec = 0;
        ...
        function updateWaitTick() {
            var elapsed = Date.now() - waitTickStartWall;
            var speed = parseFloat(Q('PlaySpeed').value);
            var recNow = waitTickStartRec + (elapsed / speed);
            if (recNow > waitTickTargetRec) recNow = waitTickTargetRec;
            var deltaTimeTotalSec = Math.floor((recNow - recFileStartTime) / 1000);
            ...
            if (recFileEndTime > recFileStartTime) {
                QS('progressbar').width = Math.floor(((recNow - recFileStartTime) / (recFileEndTime - recFileStartTime)) * 100) + '%';
            }
        }
        ...
        function processBlock(type, flags, time, data) {
            ...
            if ((waitTime < 5) || (videoWriter != null)) {
                ...
                waitTickStartWall = Date.now();
                waitTickStartRec = recFileLastTime;
                waitTickTargetRec = time;
                if (waitTickTimer) { clearInterval(waitTickTimer); }
                waitTickTimer = setInterval(updateWaitTick, 250);
                waitTimer = setTimeout(function () {
                    if (waitTickTimer) { clearInterval(waitTickTimer); waitTickTimer = null; }
                    waitTimer = null;
                    if (waitTimerArgs) { processBlockEx(waitTimerArgs[0], waitTimerArgs[1], waitTimerArgs[2], waitTimerArgs[3]); }
                }, waitTime);
            }
        }
        ...
        function processBlockEx(type, flags, time, data, forced) {
            ...
            if ((type == 2) && flagBinary && !flagUser) {
                ...
                if (recFileProtocol == 1) {
                    ...
                    if ((cmd == 27) && (cmdsize == 8)) { cmd = (view[8] << 8) + view[9]; cmdsize = view[4] * 0x1000000 + (view[5] << 16) + (view[6] << 8) + view[7]; view = view.slice(8); }
                    if (cmdsize + 8 == view.byteLength) { view = view.slice(8); } // drop injected duplicate header
                    if (cmdsize == view.byteLength) {
                        agentDesktop.ProcessBinaryCommand(cmd, cmdsize, view);
                    } else {
        ...
        function cleanup() {
            ...
            recFileEndTime = 0;
            agentTerminal = null;
            if (waitTickTimer != null) { clearInterval(waitTickTimer); waitTickTimer = null; }
        ...
        function play() {
            ...
            if (waitTimerArgs != null) {
                // Resuming mid-gap: restart timer for remaining time from current position
                var resumeArgs = waitTimerArgs;
                waitTimerArgs = null;
                processBlock(resumeArgs[0], resumeArgs[1], resumeArgs[2], resumeArgs[3]);
            } else {
        ...
        function pause() {
            ...
            if (waitTimer != null) {
                ...
                waitTimer = null;
                // Save current virtual time so resume starts from here, not from the pre-gap time
                var elapsed = Date.now() - waitTickStartWall;
                var speed = parseFloat(Q('PlaySpeed').value);
                recFileLastTime = Math.min(waitTickStartRec + (elapsed / speed), waitTickTargetRec);
                // Keep waitTimerArgs so play() can resume the gap from this point
            }
        }
        ...
        function restart() {
            ...
            QV('XTermParent', false);
            clearXTerm();
            recFileLastTime = recFileStartTime;
            if (agentDesktop) {
        ...
        function clearXTerm() {
            if (term != null) { term.dispose(); term = termimage = null; }
        }
